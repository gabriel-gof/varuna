...
- Default control plane is Zabbix API, but latest status/power hot reads can use a direct read-only PostgreSQL path against Zabbix `items` latest-value state when `ZABBIX_DB_ENABLED=1`.
...
- `GET /api/onu/{id}/power/`, `POST /api/onu/batch-power/` with `refresh=false`, and `GET /api/onu/power-report/` use a flag-driven latest-power source:
  - default: `ONU.latest_*` snapshot (`latest_onu_rx_power`, `latest_olt_rx_power`, `latest_power_read_at`),
  - canary/opt-in: live Zabbix latest values when `POWER_LATEST_READS_USE_ZABBIX=true`.
- Live Zabbix latest-power reads are DB-first: current item reads use the read-only Zabbix DB alias, and invalid-current-item fallback prefers Zabbix DB history before falling back to JSON-RPC history.
