...
- Zabbix integration uses two channels: **Zabbix DB (read-only PostgreSQL)** for ALL routine data reads (status, power, reachability, discovery fallback) when `ZABBIX_DB_ENABLED=1`, and **Zabbix API (JSON-RPC)** ONLY for commands and management (host CRUD, template linking, `task.create` for "check now", `discoveryrule.get`, `host.get`). If a DB read fails, it is logged and returns empty — there is no silent API fallback. Timeline reads (`get_history_series()` in alarm-history detail view) still use JSON-RPC as a low-frequency per-ONU path.
...
- All routine Zabbix data reads (`get_items_by_keys`, `get_latest_valid_power_history_samples`, `fetch_previous_status_samples`, `get_items_by_key_prefix`) are DB-only. Discovery control, manual `Execute now`, host management, and per-ONU alarm-history timeline queries (`get_history_series`) remain on the Zabbix JSON-RPC API path.
...
- Legacy API-only helpers `get_latest_valid_power_history_sample()` (singular) and `get_latest_item_by_key_prefix()` have been removed as dead code; their DB-backed replacements are the only active code paths.
