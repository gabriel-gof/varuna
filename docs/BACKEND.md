...
- `get_items_by_key_prefix()` queries the Zabbix DB directly (`items` + `item_rtdata` + `history_*`) to find all enabled items whose `key_` matches a given prefix via `LIKE`. Used as a rare recovery path during discovery fallback. There is no JSON-RPC API fallback -- if the DB is disabled or the query fails, the method returns an empty list.
- Removed API-only helpers that became dead code after the DB-only migration: `get_latest_valid_power_history_sample()` (singular per-item API fallback) and `get_latest_item_by_key_prefix()` (API-based prefix search). Their DB-backed replacements (`get_latest_valid_power_history_samples` plural, `get_items_by_key_prefix`) are the active code paths.
