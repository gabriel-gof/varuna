...
- Recommended practical model on one VM:
  ...
  - each client instance sets its own Zabbix host group namespace via `ZABBIX_HOST_GROUP_NAME` — title case (for example `Varuna/Gabisat`, `Varuna/Vianet`, `Varuna/Pontal`).
  - each client instance should set its own `ZABBIX_HOST_NAME_PREFIX` — ALL CAPS (for example `GABISAT-`, `PONTAL-`) so OLT host names are unique in shared Zabbix.
